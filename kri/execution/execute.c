...
/*   Updated: 2025/07/09 16:16:46 by sel-khao         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
/* int	_shell(int status, t_shell *shell, char **envp, char **str)
{
	long long	status_code;
	int			fd;
	...
	{
	    if (str[1])
			return (write(2, "bash: : too many arguments\n", 31), 1);
		printf("exit\n");
		if (check_overflow(str[0], &status_code) == 0)
		{
			write(2, "bash: exit: ", 12);
...
char	**execute(t_shell *shell, char **cmd, char *envp[])
{
	...
	// free_all(shell);//check
	//free_arr(envp, NULL); DONT cuz u r returnin it
	restore_fds(stdin_backup, stdout_backup);
	return (envp);
}
